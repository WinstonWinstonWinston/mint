...
batch = module.prior.sample(batch)
...
x_t = batch["x_t"]
x_traj = [x_t.clone()]
for t_val in tqdm(t_grid, desc="Interpolating over time" ,leave=False, position=1):
    t = t_val.repeat(B)

    # set time in the batch
    batch['t_interpolant'] = t
    x_t, z = module.interpolant.interpolate(*utils.batch2interp(batch)) 
    batch["x_t"] = x_t
    x_traj.append(batch["x_t"].clone())


X = torch.stack(x_traj, dim=0)
